...
Pool size is 20, acquisition timeout 30s, statement timeout 30s, lock timeout 15s, and idle transaction timeout 30s. The acquisition deadline also bounds waiting for a busy pool. A live free-tier burst exposed the previous 5s deadline; increasing it lets the bounded pool drain contended work without adding database connections. Known acquisition, lock and statement timeouts return HTTP 503 `database_busy`. The service does not retry transactions internally. Callers may retry with the original request and key. Contention trades latency and availability for strong money invariants. The burst runner deliberately reports HTTP failures rather than hiding them behind an automatic HTTP retry.
