...
Pool size is 20, acquisition timeout 5s, statement timeout 30s, lock timeout 15s, and idle transaction timeout 30s. The service does not retry transactions internally. Overload/database failure may produce a machine-readable HTTP error; callers may retry with the original key. Contention trades latency and availability for strong money invariants. The burst runner deliberately reports HTTP failures rather than hiding them behind an automatic HTTP retry.
