...
export async function getOrCreateWallet(
  ...
  return withPoolClient(pool, telemetry, async client => {
    const insert = () => client.query<WalletRow>(
      `
    ...
    const inserted = await (telemetry?.measure('database.wallet.insert', insert) ?? insert());
    ...
    const readExisting = () => client.query<WalletRow>(
      `
    ...
    const existing = await (telemetry?.measure('database.wallet.read', readExisting) ?? readExisting());
...
export async function getWalletForUser(
  ...
  return withPoolClient(pool, telemetry, async client => {
    const operation = () => client.query<WalletRow>(
      `
        SELECT id, user_id, balance_paise, created_at, updated_at
        FROM wallets
    ...
    const result = await (telemetry?.measure('database.wallet.read', operation) ?? operation());
